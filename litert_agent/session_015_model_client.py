...
class LiteRT015ChatClient:
    """LiteRT-LM 0.15 adapter backed by the low-level Session API.

    Conversation is used only as the official Gemma chat-template renderer.
    Live model state is held exclusively by one Session:
    ...
    def prepare_prefix(self, messages: list[dict[str, str]]) -> WarmResult:
        ...
        try:
            ...
            # This Conversation is a renderer only. With the stock 0.15 API it
            # performs no prefix prefill during creation.
            ...
            # Rendered strings already contain the model chat template.
            ...
            LOGGER.info(
                "litert015-session-warm %s resident=%d elapsed=%.3fs",
                self.label,
            ...
            self.close()
            raise ModelClientError(
                f"LiteRT-LM 0.15 Session prefix prefill failed: {exc}"
            ) from exc

    def chat(self, messages: list[dict[str, str]]) -> ChatResponse:
        if not messages or messages[-1].get("role") != "user":
            raise ModelClientError(
                "LiteRT Session turn must end in a user message"
            )
        ...
        # Exact state continuity only: never silently rebuild semantic history.
        ...
        try:
            ...
            LOGGER.info(
                "litert015-session-kv %s resident=%d new=%d decode=%d "
            ...
            raise ModelClientError(
                f"LiteRT-LM 0.15 Session request failed: {exc}"
            ) from exc
    ...
    def _configure_renderer_ffi(self) -> None:
        ...
        self._lib.litert_lm_conversation_render_preface_to_string.restype = (
            ctypes.c_char_p
        )
        ...
        self._lib.litert_lm_conversation_render_message_to_string.restype = (
            ctypes.c_char_p
        )
...
def _session_response_text(response) -> str:
    ...
    if not texts:
        raise ModelClientError(
            f"LiteRT Session decode returned no text: {response!r}"
        )
