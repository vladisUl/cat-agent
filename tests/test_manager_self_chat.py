...
class ManagerSelfChatTest(unittest.TestCase):
    ...
    def test_self_protocol_and_direct_command(self) -> None:
        item = parse_manager_output("SELF shell")
        self.assertEqual(item.action, ManagerAction.SELF)
    ...
    def test_manager_self_uses_own_model_context_and_not_agent(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertIn("SELF_READY", client.calls[1][-1]["content"])
            self.assertIn('"name": "shell"', client.calls[1][-1]["content"])
            self.assertIn("self-ok", client.calls[2][-1]["content"])
